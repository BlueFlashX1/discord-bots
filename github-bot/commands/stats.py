...
class StatsCommand(commands.Cog):
    ...
    @app_commands.command(name="stats", description="Get GitHub contribution statistics")
    @app_commands.describe(username="GitHub username (defaults to your configured username)")
    async def stats(self, interaction: discord.Interaction, username: str = None):
        ...
        try:
            ...
            embed = create_error_embed(f"Error fetching stats: {str(e)}")
            await interaction.followup.send(embed=embed)
