...
// Save journal entry
async function saveJournal(content, dateStr) {
  const [year, month] = dateStr.split('-');
  const filename = `${dateStr}.md`;

  try {
    // Use transfer utility to save and optionally transfer
    const savedPath = await transfer.saveAndTransfer(content, `journal/${year}/${month}/${filename}`, 'journal');
    console.log(`Journal saved to: ${savedPath}`);
    return savedPath;
  } catch (error) {
...
async function main() {
  ...
  // Save journal
  const journalPath = await saveJournal(journalContent, targetDate);
